...
void KalmanFilter::initMatrices(const VectorXd &pos, const VectorXd &accel, const VectorXd &dir, double speed)
{
	...
	this->Q << q_pos * q_pos * VARIANCE_ACCEL, 0, 0, q_pos * DT * VARIANCE_ACCEL, 0, 0,
...
void KalmanFilter::plotvariance() const
{
	// Print the state covariance matrix
	std::ofstream varFile("variance.txt", std::ios::app);
	for (int i = 0; i < this->P.rows(); ++i)
	{
		varFile << this->P(i, i) << " "; // Write each variance (diagonal element)
	}
	varFile << std::endl;
	varFile.close();
}